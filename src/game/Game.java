...
public class Game {
    ...
    private boolean stage2_FindKey(int i) {
        System.out.println("\n=== 2단계: 방 탈출 ===");
        ...
        while (true) {
            ...
            switch (choice) {
                case "1":
                    System.out.println("서랍 안에서 1번 열쇠를 발견했습니다!");
                    player.getInventory().addItem("1번 열쇠");
                    break;
                case "2":
                    System.out.println("벽면의 그림 뒤에서 3번 열쇠를 발견했습니다!");
                    player.getInventory().addItem("3번 열쇠");
                    break;
                case "3":
                    System.out.println("침대 아래에서 2번 열쇠를 발견했습니다!");
                    player.getInventory().addItem("2번 열쇠");
                    break;
                default:
                    System.out.println("올바른 번호를 입력하세요.");
                    continue;
        ...
        while (true) {
            System.out.print("사용할 열쇠 번호를 입력하세요 (1-3): ");
            try {
                int choice = scanner.nextInt();
                scanner.nextLine();

                if (choice >= 1 && choice <= 3) {
                    String selectedKey = (choice) + "번 열쇠";

                    if (player.useKey(selectedKey, gameItems)) {
                        System.out.println("문이 열렸습니다!");
                        return true;
                    }
                } else {
                    System.out.println("1-3 사이의 번호를 입력하세요.");
                }
            } catch (Exception e) {
                System.out.println("올바른 번호를 입력하세요.");
                scanner.nextLine();
    ...
    private boolean stage5_FinalEscape() {
        ...
        if (baseBallGame.playBaseBallGame(scanner)) {
            System.out.println("\n🎉 게임 클리어! 🎉");
            System.out.println("당신은 성공적으로 살인자의 저택에서 탈출했습니다!");
            return true;
        } else {
            player.die();
            return false;
